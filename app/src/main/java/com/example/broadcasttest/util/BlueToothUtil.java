...
public class BlueToothUtil {
    ...
    public static byte[] hexStringToByte(String hex) {
        ...
        for (int j = 0; j< result.length; j++) {
            System.out.println("结果数组" + result[j]);
        }
        return result;
    }
    ...
    public static byte[]  fieldShaping(SendMessage sendMessage) {
        ...
        //加上CRC的报文
//      return hexStringToByte(str+getCRC3(hexStringToByte(str)));
    }
    ...
    public static boolean initTest(String msg, String targetMac) {
        ...
        Log.d("TAG", "initTest: 是否相同 =" + mac.equals(targetMac));
     ...
     * @return
     */
    private static String hexPad(String hex,int len) {
        String zero = "";
        for (int i = 0; i < len; i++) {
            zero += "00";
        }
        int tem = 2*len-hex.length();
        return zero.substring(0,tem) + hex;
    }

    /**
     * 判断报文是否为当前所需报文
     * @param receiveMessage 接收到的报文对象
     * @param curNum    当前的数据编号
     * @param mac 当前的mac
     * @param curType 当前的命令类型
     * @return  正确与否
     */
